...
class PlayState extends MusicBeatState {
	...
	var movePoint:FlxPoint;
	var staticPoint:FlxPoint;
	...
	public var clockBase:FlxSprite;
	public var clockArm:FlxSprite;
	...
	public var iconP1:MadoIcon;
	public var iconP2:MadoIcon;
	...
	override public function create() {
		...
		var bfMid = boyfriend.getGraphicMidpoint();
		var dadMid = dad.getGraphicMidpoint();
		//camFollow.setPosition((dadMid.x + bfMid.x)/2, dadMid.y); // holy shit midpoint formula

		var camPos:FlxPoint = FlxPoint.get((dadMid.x + bfMid.x)/2, dadMid.y - 35);
		...
		clockBase = new FlxSprite().loadGraphic(Paths.image('gameplay_UI/clock/clock_base'));
		clockBase.scale.set(1.5, 1.5);
		clockBase.updateHitbox();
		clockBase.setPosition(20, (!ClientPrefs.data.downScroll) ? 20 : (FlxG.height - clockBase.height) - 20);
		clockBase.alpha = 0;
		clockBase.antialiasing = ClientPrefs.data.antialiasing;
		uiGroup.add(clockBase);

		clockArm = new FlxSprite().loadGraphic(Paths.image('gameplay_UI/clock/clock_arm'));
		clockArm.scale.set(1.5, 1.5);
		clockArm.updateHitbox();
		clockArm.setPosition(20, (!ClientPrefs.data.downScroll) ? 20 : (FlxG.height - clockBase.height) - 20);
		clockArm.alpha = 0;
		clockArm.antialiasing = ClientPrefs.data.antialiasing;
		uiGroup.add(clockArm);
		...
		staticPoint = new FlxPoint(camFollow.x, camFollow.y);
		movePoint = new FlxPoint(camFollow.x, camFollow.y);
		...
		var healthHeight = FlxG.height * (!ClientPrefs.data.downScroll ? 0.87 : 0.10);

		healthBar = new Bar(0, healthHeight, 'healthBar', function() return health, 0, 2);
		...
		healthBarOverlay = new FlxSprite(0, healthHeight).loadGraphic(Paths.image('gameplay_UI/bars/healthBar-overlay'));
		...
		uiGroup.add(healthBarOverlay);

		iconP1 = new MadoIcon(boyfriend.healthIcon, true);
		iconP1.visible = !ClientPrefs.data.hideHud;
		iconP1.alpha = ClientPrefs.data.healthBarAlpha;
		iconP1.changeIconMood('neutral');
		iconP1.setPosition(((FlxG.width - healthBarOverlay.x) + (healthBarOverlay.x / 2 - iconP1.width / 2)), ((healthBarOverlay.y + (healthBarOverlay.height / 2)) - iconP1.height / 2) - 15);
		uiGroup.add(iconP1);

		iconP2 = new MadoIcon(dad.healthIcon, false);
		iconP2.visible = !ClientPrefs.data.hideHud;
		iconP2.alpha = ClientPrefs.data.healthBarAlpha;
		iconP2.changeIconMood('neutral');
		iconP2.setPosition((healthBarOverlay.x / 2) - iconP2.width / 2, ((healthBarOverlay.y + (healthBarOverlay.height / 2)) - iconP2.height / 2) - 15);
		uiGroup.add(iconP2);

		scoreTxt = new FlxText(0, healthBar.y + healthBar.height + 14, FlxG.width, "", 32);
		scoreTxt.setFormat(Paths.font("rpg.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		countdownSprite.frames = Paths.getSparrowAtlas('gameplay_UI/count/countdown');
		countdownSprite.animation.addByPrefix('one', 'one', 24, false);
		countdownSprite.animation.addByPrefix('two', 'two', 24, false);
		countdownSprite.animation.addByPrefix('three', 'three', 24, false);
		countdownSprite.animation.addByPrefix('go', 'go', 24, false);
		countdownSprite.animation.play('three');
		goPos = countdownSprite.getPosition();
		countdownSprite.updateHitbox();
		uiGroup.add(countdownSprite);

		countdownSprite.setPosition(10, (!ClientPrefs.data.downScroll) ? 10 : (FlxG.height - countdownSprite.height) - 10);
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer) {
				...
				switch (swagCounter) {
					case 0:
						countdownSprite.animation.play('three');
						countdownSprite.updateHitbox();
						FlxG.sound.play(Paths.sound('gameplay/mado/count'), 0.6);
						tick = THREE;
					case 1:
						countdownSprite.animation.play('two');
						countdownSprite.updateHitbox();
						FlxG.sound.play(Paths.sound('gameplay/mado/count'), 0.6);
						tick = TWO;
					case 2:
						countdownSprite.animation.play('one');
						countdownSprite.updateHitbox();
						FlxG.sound.play(Paths.sound('gameplay/mado/count'), 0.6);
						tick = ONE;
					case 3:
						countdownSprite.animation.play('go');
						countdownSprite.y -= countdownSprite.height + 18; //double it and give it to the next person
						countdownSprite.updateHitbox();
						FlxG.sound.play(Paths.sound('gameplay/mado/go'), 0.6);
						freezeCamera = false;
						tick = GO;
					case 4:
						moveCameraSection();
	...
	function startSong():Void {
		...
		FlxTween.tween(clockBase, {alpha: 1}, 1);
		FlxTween.tween(clockArm, {alpha: 1}, 1);
		...
		if (autoUpdateRPC)
			DiscordClient.changePresence(detailsText, null, iconP2.getCharacter(), true, songLength);
		#end
	...
	private function generateStaticArrows(player:Int):Void {
		...
		for (i in 0...4) {
			...
			babyArrow.postAddedToGroup(playerStrums.length);
		}
	}
	...
	override public function onFocusLost():Void {
		...
		if (health > 0 && !paused && autoUpdateRPC)
			DiscordClient.changePresence(detailsPausedText, null, iconP2.getCharacter());
		#end
	...
	function resetRPC(?showTime:Bool = false) {
		...
		if (showTime)
			DiscordClient.changePresence(detailsText, null, iconP2.getCharacter(), true,
				songLength
				- Conductor.songPosition
				- ClientPrefs.data.noteOffset);
		else
			DiscordClient.changePresence(detailsText, null, iconP2.getCharacter());
		#end
	}
	...
	override public function update(elapsed:Float) {
		...
		camFollow.setPosition(movePoint.x, movePoint.y);
	...
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		//.animation.curAnim.curFrame = (healthBar.percent < 20) ? 1 : 0; // If health is under 20%, change player icon to frame 1 (losing icon), otherwise, frame 0 (normal)
		//iconP2.animation.curAnim.curFrame = (healthBar.percent > 80) ? 1 : 0; // If health is over 80%, change opponent icon to frame 1 (losing icon), otherwise, frame 0 (normal)
		if (healthBar.percent > 80) {
			iconP2.changeIconMood('losing');
			iconP1.changeIconMood('winning');
		} else if (healthBar.percent < 20) {
			iconP2.changeIconMood('winning');
			iconP1.changeIconMood('losing');
		} else {
			iconP2.changeIconMood('neutral');
			iconP1.changeIconMood('neutral');
		}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead) {
			...
			if (ret != LuaUtils.Function_Stop) {
				...
				if (autoUpdateRPC)
					DiscordClient.changePresence("Game Over - " + null, SONG.song, iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch (eventName) {
				...
				switch (charType) {
					case 0:
						if (boyfriend.curCharacter != value2) {
							...
							iconP1.makeIcon(boyfriend.healthIcon);
						}
					...
					case 1:
						if (dad.curCharacter != value2) {
							...
							dad.alpha = lastAlpha;
							iconP2.makeIcon(dad.healthIcon);
						}
			...
			case 'Move Camera':
				moveCamera(value1.toLowerCase() == 'true');
		}
	...
	public function moveCamera(isDad:Bool) {
		...
		staticPoint.set(camFollow.x, camFollow.y);
	}
	...
	function opponentNoteHit(note:Note):Void {
		...
		if (!note.isSustainNote && !SONG.notes[curSection].mustHitSection) {
			switch (note.noteData) {
				case 0:
					movePoint.set(staticPoint.x - 18, staticPoint.y);
				case 1:
					movePoint.set(staticPoint.x, staticPoint.y +  18);
				case 2:
					movePoint.set(staticPoint.x, staticPoint.y -  18);
				case 3:
					movePoint.set(staticPoint.x +  18, staticPoint.y);
			...
			new FlxTimer().start(1, (t)->{
				camFollow.setPosition(staticPoint.x, staticPoint.y);
			});
		}
	...
	override function beatHit() {
		...
		if (curBeat % 2 == 0) {
			...
			var totalLength = inst.length;

			var percentageCompleted = curPos / totalLength;
			var angle = 360 * percentageCompleted; // this should be a float like 0.8 (80%)
			clockArm.angle = angle;
		}
	...
	public function initLuaShader(name:String, ?glslVersion:Int = 120) {
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		addTextToDebug('PS: Missing shader $name .frag AND .vert files!', FlxColor.RED);
		#else
